...
banner(){
  ...
  echo "╔════════════════════════════════════════════════════════╗"
  echo "║                                                        ║"
  echo "║             ✦  A N I M E   C L O U D  ✦               ║"
  echo "║                 3D NEON EDITION V3                    ║"
  echo "║                                                        ║"
  echo "╚════════════════════════════════════════════════════════╝"
  echo -e "${R}"
  echo -e "${M}${B}        █████╗ ███╗   ██╗██╗███╗   ███╗███████╗${R}"
  ...
  echo
  echo -e "${C}${B}╭────────────────────────────────────────────────────────╮${R}"
  echo -e "${C}${B}│  ☁ PTERODACTYL   ⚙ WINGS   ☁ CLOUDFLARE   🐳 DOCKER │${R}"
  echo -e "${M}${B}│  ◆ NEON UI        ◆ SSL     ◆ FIREWALL      ◆ AUTO    │${R}"
  echo -e "${C}${B}╰────────────────────────────────────────────────────────╯${R}"
  printf "${C}${B}  3D interface ${R}"
  for _ in 1 2 3 4 5 6; do printf "${M}◆${R}"; sleep 0.04; done
  echo -e " ${G}${B} ONLINE${R}"
  echo
}
root(){ [[ $EUID -eq 0 ]] || die 'Run as root.'; }
os(){ source /etc/os-release; [[ $ID == ubuntu || $ID == debian ]] || die 'Ubuntu/Debian required.'; ok "OS: $PRETTY_NAME"; }
info(){ banner; echo -e "${M}${B}SYSTEM INFORMATION${R}"; echo "Hostname: $(hostname)"; echo "Kernel: $(uname -r)"; echo "CPU: $(nproc) cores"; echo "RAM: $(free -h|awk '/Mem:/{print $3" / "$2}')"; echo "Disk: $(df -h /|awk 'NR==2{print $3" / "$2" ("$5")"}')"; echo "IPv4: $(curl -4 -fsS --max-time 5 https://api.ipify.org || echo unknown)"; for s in docker nginx mariadb redis-server pteroq wings cloudflared; do systemctl is-active --quiet "$s" 2>/dev/null && echo -e "${G}● RUNNING${R} $s" || echo -e "${RED}● STOPPED${R} $s"; done; pause; }
...
full(){ banner; read -rp 'Start FULL AUTO INSTALL? [y/N]: ' A; [[ "$A" =~ ^[Yy]$ ]] || return; neon_progress 'Initializing Anime Cloud 3D'; deps; php; composer; database; panel; panelcfg; nginx; queue; docker; wings; tunnel; ssl; firewall; admin; health; banner; echo -e "${G}${B}✔ INSTALLATION COMPLETE${R}"; pause; }
menu(){ while true; do banner; echo -e "${C}${B}╭──────────── 🚀 DEPLOYMENT & SERVICES ────────────────╮${R}"; echo -e "│ ${G}[1]${R} 🚀 FULL AUTO INSTALL"; echo -e "│ ${G}[2]${R} 🖥️ VPS / SYSTEM INFORMATION"; echo -e "│ ${G}[3]${R} ⚙️ WINGS AUTO SETUP"; echo -e "│ ${G}[4]${R} 🦎 PTERODACTYL PANEL"; echo -e "│ ${G}[5]${R} 🐳 DOCKER"; echo -e "│ ${G}[6]${R} 📊 SERVICE MONITOR"; echo -e "│ ${G}[7]${R} 📦 CONTAINER TOOLS"; echo -e "${C}${B}╰────────────────────────────────────────────────────────╯${R}"; echo; echo -e "${M}${B}╭────────────── ☁ CLOUD & SECURITY ────────────────────╮${R}"; echo -e "│ ${M}[8]${R} ☁️ CLOUDFLARE TUNNEL"; echo -e "│ ${M}[9]${R} 🌐 CLOUDFLARE DNS"; echo -e "│ ${M}[10]${R} 🔒 LET'S ENCRYPT SSL"; echo -e "│ ${M}[11]${R} 🛡️ FIREWALL"; echo -e "│ ${M}[12]${R} 🔍 HEALTH CHECK"; echo -e "${M}${B}╰────────────────────────────────────────────────────────╯${R}"; echo; echo -e "${Y}${B}╭──────────────── ✨ TOOLS ─────────────────────────────╮${R}"; echo -e "│ ${Y}[13]${R} ✨ AUTO REPAIR / REINSTALL"; echo -e "│ ${Y}[14]${R} 📝 INSTALL LOG"; echo -e "│ ${RED}[0]${R} ⏻ EXIT"; echo -e "${Y}${B}╰────────────────────────────────────────────────────────╯${R}"; echo; echo -ne "${C}${B}╭─[ Anime Cloud 3D ]─➜ ${R}"; read -r O; case "$O" in 1) full;; 2|6) info;; 3) wings; pause;; 4) panel; panelcfg; nginx; queue; pause;; 5|7) docker; pause;; 8) tunnel; pause;; 9) warn 'Configure Cloudflare DNS/API separately.'; pause;; 10) ssl; pause;; 11) firewall; pause;; 12) health;; 13) full;; 14) clear; less "$LOG";; 0) clear; exit 0;; *) warn 'Invalid option.'; sleep 1;; esac; done; }
root; os; menu
